...
public class DonorReceiver extends User {
   ...
   * @param DoD the date of death of the donor (if applicable), a local date object
   ...
   */
  public DonorReceiver(String firstName, String middleName, String lastName, LocalDate DOB,
      String nhi, ContactDetails details,
      LocalDate DoD, char gender, char birthGender, String bloodType, double height,
      double weight) {
  ...
  public DonorReceiver(
    ...
    super(firstName, middleName, lastName, contactDetails, userName, password, creationDate,
        modifications); //Need a json constructor in the superclasses
    if (userAttCol == null) {
  ...
  public String toString() {
    ...
    detailsString += String
        .format("\tDonorReceiver created at: %s\n", formatDateTimeToString(creationTimeStamp));
    if (dateOfBirth != null) {
      detailsString += String.format("\tDate of birth: %s\n", formatDateToString(dateOfBirth));
    }
    if (dateOfDeath != null) {
  ...
  public String updateTitle(User modifyingAccount, String value) {
    String message =
        "ERROR: Invalid value " + value + ". Title must have at most 10 alphabetical characters.\n";
    if (UserValidator.validateAlphanumericString(false, value, 0, 10)) {

      if (title != null) {
        LogEntry logEntry = new LogEntry(this, modifyingAccount, "title", title,
            value.toUpperCase());
        addToSystemLog(logEntry);
        updateLog.add(logEntry);
        message = logEntry.toString();
      } else {
        LogEntry logEntry = new LogEntry(this, modifyingAccount, "title", "unspecified",
            value.toUpperCase());
        addToSystemLog(logEntry);
        updateLog.add(logEntry);
        message = logEntry.toString();
  ...
  public String updateGender(User modifyingAccount, String value) {
    ...
    if (UserValidator.validateGender(value.charAt(0))) {
      if ((int) gender != 0) {
  ...
  public String updateDateOfBirth(User modifyingAccount, String value) {
    LocalDate date;
    String message =
        "ERROR: Invalid value " + value + ". Date of birth must be a valid date that is before the "
            +
  ...
  public String updateDateOfDeath(User modifyingAccount, String value) {
    ...
    String message =
        "ERROR: Invalid value " + value + ". Date of death must be a valid date that is before the "
            +
            "current date "
    ...
    if (value.equalsIgnoreCase("")) {
      LogEntry logEntry = new LogEntry(this, modifyingAccount, "dateOfDeath",
          formatDateToString(dateOfDeath), "unspecified");
      addToSystemLog(logEntry);
      updateLog.add(logEntry);
      message = logEntry.toString();
      setDateOfDeath(null);
      ...
      if (UserValidator.validateDateOfDeath(date, dateOfBirth)) {
        if (dateOfDeath != null) {
          LogEntry logEntry = new LogEntry(this, modifyingAccount, "dateOfDeath",
              formatDateToString(dateOfDeath), value);
          addToSystemLog(logEntry);
          updateLog.add(logEntry);
          message = logEntry.toString();
        } else {
  ...
  public String updateContact(User modifyingAccount, String attribute, String value) {
    ...
    switch (attribute) {
      ...
      case "addressStreet": {
        oldValue = address.getStreetAddressLn1();
        message = address.updateStreetAddressLine1(value);
        break;
      }
      case "addressCity": {
      ...
      case "emergAddressStreet": {
        oldValue = emergencyAddress.getStreetAddressLn1();
        message = emergencyAddress.updateStreetAddressLine1(value);
        break;
      }
   ...
   * @return returns 'true' if other's NHI code is the same as this Accounts nhi code. 'false'
   * otherwise
   */
  ...
  /**
   * Attempts to update one of the DonorReceiver's Illness objects attributes with the given
   * parameters. It first checks that if the 'name' attribute is being updated then there will be no
   * resulting duplicate Illness's in the Illness list. Then It searches the Illness list to find
   * the Illness matching the given name string and attempts to update that Illness. If the Illness
   * is not found, or if the new given value of the given attribute is invalid then the
   * DonorReceiver updateMessage attribute is updated with an error message. Otherwise the
   * DonorReceiver's updateLog is updated with the log of the updated attribute.
   *
   * @param name A string representing the name of the Illness to update (should match a name in the
   * master Illness list).
   * @param attribute A string of the Illness attribute to update. Should be one of 'name', 'date',
   * 'chronic', or 'cured'.
  ...
  public void updateIllness(User modifyingAccount, String name, String attribute, String value) {
    ...
    String[] result = null;
    if (attribute
        .equals("name")) { // check that the edited name does not already exist in the illness list.
      for (Illness illness : masterIllnessList) {
        if (value.toUpperCase().equals(illness.getName())) {
          duplicate = true;
          message = String.format(
              "ERROR: Illness with name %s already exists in the diagnoses lists, cannot add duplicate.",
              value);
          break;
        }
      }
        ...
        if (description.equals(procedure.getDescription())) {
          ...
          if (affectedOrgans.equals(procedureOrgans)) {
            if (dateString == null && procedure.getDate() == null) {
              duplicate = true;
            } else if (dateString != null && procedure.getDate() != null && dateString
                .equals(formatDateToStringSlash(procedure.getDate()))) {
              duplicate = true;
            }
          }
        }
   ...
   * #procedureAlreadyExistsErrorMessage} if the account already has a procedure with details the
   ...
   * "Cornea", "Middle Ear", "Skin", "Bone", "Bone Marrow", and "Connective Tissue".
   * @param modifyingAccount account to be modified
   * @throws Exception Throws exception with message {@link #procedureDoesNotBelongErrorMessage} if
   * the procedure is not in the list of medical procedures for this account. Throws exception with
   * message {@link #procedureAlreadyExistsErrorMessage} if the account already has a procedure with
  ...
  public void updateMedicalProcedure(User modifyingAccount, MedicalProcedure procedure,
    ...
    if (!medicalProcedureBelongsToAccount(procedure)) {
      throw new Exception(procedureDoesNotBelongErrorMessage);
    }
