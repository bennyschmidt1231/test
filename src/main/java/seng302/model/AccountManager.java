...
public class AccountManager {
    ...
    @JsonIgnore private Map<String, DonorReceiver> donorReceivers = new LinkedHashMap();
    @JsonIgnore private Map<String, DonorReceiver> donorReceiverMasterList = new LinkedHashMap();

    @JsonIgnore private Map<String, Clinician> clinicians = new LinkedHashMap();
    @JsonIgnore private Map<String, Clinician> clinicianMasterList = new LinkedHashMap();
    @JsonIgnore public Map<String, Clinician> clinicianTwilightZone = new LinkedHashMap();

    @JsonIgnore private Map<String, Administrator> administrators = new LinkedHashMap();
    @JsonIgnore private Map<String, Administrator> administratorMasterList = new LinkedHashMap();
    @JsonIgnore public Map<String, Administrator> adminTwilightZone = new LinkedHashMap();
...
//---------------------------------------------------------------------------------------
    ...
    private void logDonorReceiverChange(User modifyingAccount, DonorReceiver donorReceiver, String valChanged, String originalVal, String changedVal) {
        ...
        LogEntry logEntry = new LogEntry(donorReceiver, modifyingAccount, valChanged, originalVal, changedVal);
    ...
    /**
     * Checks if the NHI given is already in use within the database (NHI is the same as the username). Returns true if
     * NHI exists; false if does not exist.
    ...
    public boolean checkUsedNHI(String NHI) {
        // NOTE: NHI is the same as the username
    ...
    public boolean validateNHI(String NHI) {
        ...
        person = donorReceiverMasterList.get(NHI);
        if (person != null) {
            if (!person.isActive()) {
  ...
  public boolean checkNHIRegex(String NHI) {
    ...
    /**
     * Searches for accounts by name. Returns an arraylist containing matching
    ...
    public ArrayList<DonorReceiver> getAccountsByName(String value) {
        ...
        if (value.equals("")) {
            ...
            return new ArrayList<DonorReceiver>(donorReceivers.values());
    ...
    /**
     * Searches for accounts by NHI. Returns an ArrayList containing all matching
    ...
    /**
     * updates the master list of accounts with all those in the Accounts array and in the Twilight array. Thus
     * The old account records will be overwritten by any account with saved changes. Note that the logs of reactivated
     * accounts will be merged with logs of new changes.
     */
     ...
     *
     * @param donorReceiver The account we want to check is, or is not, in the transplant waiting list
    ...
    /**
     * Verifies the transplant waiting list is up-to-date. Checks each DonorReceiver on the transplant waiting list is
     * actually waiting for a transplant.
     */
    ...
    /**
     * Adds a clinician object to the clinicians linked hashmap.
     * @param clinician A clinician administrator object.
     */
    public void addToClincians(Clinician clinician) {
        this.clinicians.put(clinician.getUserName(), clinician);
    }


    /**
     * Checks if the clinicians array list is empty, and if it is, populates it with a default clinician object.
     * This ensures that there is always a clinician object in the the clinicians array list upon application start up.
     */
    ...
    public boolean checkStaffIDIsNotUsed(String id) {
        Clinician person = clinicians.get(id);
        if (person != null) {
            // ID already in use; cannot use again
            return false;
        } else {
            return true;
        }
    ...
    /**
     * Inserts an admin into the database administrator list.
     * If the username is already in use, that administrator will be overwritten if with the given administrator only if
     * 'replace' is set to 'True'. If it is set to 'False', no changes will be made.
     * Precondition: IT IS UP TO THE CALLER OF THIS FUNCTION TO ALREADY KNOW IF THE ADMIN EXISTS OR NOT AND SET 'REPLACE' APPROPRIATELY.
     ...
     * @param replace A boolean to specify whether or not to overwrite an administrator if the username is already in use.
     */
    ...
    /**
     * Inserts a clinician into the database clinicians list.
     * If the username is already in use, that clinician will be overwritten by the given clinician only if 'replace'
     * is set to 'True'. If it is set to 'False', no change will be made.
     * Precondition: IT IS UP TO THE CALLER OF THIS FUNCTION TO ALREADY KNOW IF THE CLINICIAN EXISTS OR NOT AND SET 'REPLACE' APPROPRIATELY.
     ...
     * @param replace A boolean to specify whether the clinician list will be searched in order to replace the existing clinician.
     */
    ...
    /**
     * Inserts a donor/receiver into the database donorReceiver list.
     * If the username is already in use, that donor/receiver will be overwritten only if 'replace' is set to True.
     * If it is set to False, no change will be made.
     * Precondition: IT IS UP TO THE CALLER OF THIS FUNCTION TO ALREADY KNOW IF THE DONOR EXISTS OR NOT AND SET 'REPLACE' APPROPRIATELY.
     * @param donorReceiver A donorReceiver object to be added to the database.
     * @param replace A boolean to specify whether the donor list will be searched in order to replace the existing donor.
     */
