...
import seng302.model.comparitors.AgeComparator;
import seng302.model.comparitors.CityComparator;
import seng302.model.comparitors.DonorReceiverStatusComparator;
import seng302.model.comparitors.GenderComparator;
import seng302.model.comparitors.NameAlphabeticalComparator;
import seng302.model.comparitors.NameComparator;
import seng302.model.comparitors.RegionComparator;
...
public class DonorListController extends ListController {
   ...
   */
  @FXML
  private TableColumn<DonorReceiver, Character> genderColumn;
   ...
   */
  @FXML
  private TableColumn<DonorReceiver, String> cityColumn;
   ...
   */
  @FXML
  private CheckComboBox filterBirthGender;
   ...
   */
  @FXML
  private CheckComboBox filterReceiverOrgans;
   ...
   */
  @FXML
  private CheckComboBox filterDonorOrgans;
   ...
   */
  @FXML
  private CheckComboBox filterRegion;
   ...
   */
  @FXML
  private CheckComboBox filterStatus;
   ...
   */
  private ArrayList<CheckComboBox> checkComboBoxes = new ArrayList<>();
  ...
  private void deleteSelected() {
    ...
    if (selectedAccount != null) {
      ...
      if (result.get() == ButtonType.OK) {
        ...
        accountManager.issueDeleteWithoutConfirmation(AccountManager.getCurrentUser(),
            selectedAccount.getUserName());
...
/*        searchField.textProperty().addListener((observable, oldValue, newValue) -> {
  ...
  public String getAccountStatus(DonorReceiver account) {
    try {
      ...
      if (donatingString.equals("Organs to receive:\nNo organs to receive\n\n")) {
        isDonating = false;
      }
  ...
  /**
   * Returns true if the given account has an greater than or equal to min and less than or equal to
   * max. If max/min arent present then true. Otherwise false.
   *
   * @param account account to be filtered
   * @return true if age is in the given range, false otherwise.
   */
  public boolean accountAgeRange(DonorReceiver account) {
    try {
      ...
      return false; // If NPE thrown then return false so that filtering isnt broken
    }
   ...
   */
  boolean accountFromRegion(DonorReceiver account, ObservableList<String> region) {
    if (region.contains("Any Region")) {
      return true;
    } else {
      try {
        if (account.getAddressRegion() != null) {
          return region.contains(account.getAddressRegion());
        } else {
          return false;
        }
      } catch (NullPointerException e) {
        return false; // If NPE thrown when getting region assume false
      }
    }
  }
   ...
   */
  public boolean accountStatus(DonorReceiver account, ObservableList<String> donorReceiverStatus) {
    try {
      if (donorReceiverStatus.contains("Any Status")) {
        return true;
      } else if ((donorReceiverStatus.contains("Neither")) && (getAccountStatus(account)
          .equalsIgnoreCase(""))) {
        return true;
      } else {
  ...
  /**
   * Returns true if the full name of the given account contains the given string. Returns false
   * otherwise. Both the full name and string will be converted to lowercase during the comparison.
   *
  ...
  private void populateFilterOptions() {
    ...
    checkComboBoxes.addAll(Arrays
        .asList(filterBirthGender, filterReceiverOrgans, filterDonorOrgans, filterRegion,
            filterStatus));
