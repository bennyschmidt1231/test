...
public class CreateUserPaneController {
  ...
  /**
   * Creates a new Donor DonorReceiver and adds it to the database from the provided details. The
   * details provided are: NHI code, given name, last name, and date of birth. The account is not
  ...
  void createAccount(ActionEvent event) {
    boolean testNHI = db.validateNHI((usernameTextField.getCharacters().toString()));
    if (!testNHI) {
      showBadNHIMessage();
      return;
    }
    ...
    boolean testPassword = UserValidator.validateAlphanumericString
        (false, passwordField.getCharacters().toString(), 1, 50);
    boolean testConfirmPassword = UserValidator.validateAlphanumericString
    ...
    try {
      ...
      testDOB = UserValidator.validateDateOfBirth(localDate);
    } catch (Exception e) {
      valid = false;
    }
    ...
    DonorReceiver donorReceiver = new DonorReceiver(givenNameTextField.getCharacters().toString(),
        "",
        lastNameTextField.getCharacters().toString(), date,
        usernameTextField.getCharacters().toString());
    donorReceiver.setPassword(password);

    // Look for the typed username in the donorReceivers map
    DonorReceiver acc = db.getDonorReceiverByUsername(donorReceiver.getUserName());
    if (acc != null) {
  ...
  public void showBadNHIMessage() {
    ...
    alert.setContentText(
        "Either you have entered an invalid NHI code or there is already an account with that code. "
            +
  ...
  public void showBadGivenNameMessage() {
    ...
    alert.setContentText(
        "Given name must be between 1 and 50 alphabetical characters. Spaces, commas, apostrophes,"
            +
  ...
  public void showBadDateMessage() {
    ...
    alert.setContentText(
        "Date of birth must be a valid date that is before the current date. The date also has to "
            +
  ...
  public void showSuccessMessage() {
    ...
    alert.setContentText(String.format(
        "DonorReceiver created. Remember to save the application to make the account permanent."));
    alert.getDialogPane().setMinHeight(Region.USE_PREF_SIZE);
    alert.showAndWait();
  ...
  public void showRecreationMessage() {
    ...
    alert.setContentText(String
        .format("WARNING: NHI code already used for deactivated account. Reactivated account. " +
