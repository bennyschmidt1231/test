...
public class createOrModifyIllnessController {
  ...
  /**
   * Checks if the updateMessage attribute of the current clinician being viewed starts with the
   * word 'ERROR'.
   *
  ...
  /**
   * A success dialog alert box given if the inputted values are all correct. The given message is
  ...
  /**
   * Checks if the GUI text field dateOfDiagnoses has been changed, and if it has attempts to update
   * the Illness date attribute.
   *
   * @return Returns 'true' only if the Illness date attribute has been successfully changed,
   * 'false' otherwise.
   */
  public boolean checkDate() {
    ...
    if (!dateOfDiagnoses.getValue().equals(illness.getDate())) {
      ...
      try {
        ...
        donor.updateIllness(AccountManager.getCurrentUser(), illness.getName(), "date",
            DonorReceiver.formatDateToString(date));
        if (!updateMessageIsAnErrorMessage()) {
          changes = true;
        }
      } catch (Exception e) {
        showBadInputMessage(String
            .format("ERROR: Invalid value %s. Date must be a valid date that is not after the " +
                    ...
                    +
                    "'dd/MM/YYYY'.\n", dateOfDiagnoses.getValue(),
                illness.formatDate(donor.getDateOfBirth())));
      }
    }
    return changes;
  }
  ...
  /**
   * Checks if the curedCheckBox and/or chronicCheckBox has been changed and if (they) have,
   * attempts to update the Illness object cured and/or chronic attributes. Order of resolution is
   * important here and the calls to this function should NOT be changed or modified.
   *
   * @return Returns 'true' if the illness object's cured and/or chronic attributes have been
  ...
  public boolean checkCured() {
    ...
    if (curedChanged && chronicChanged
        && curedIsTrue) { // chronic is becoming true and cured is becoming false
    ...
    } else if (curedChanged
        && chronicChanged) { // chronic is becoming false and cured is becoming true
      checkChronic();
      if (!updateMessageIsAnErrorMessage()) {
        changes = true;
      }
  ...
  /**
   * Checks if the chronicCheckBox has been changed, and if it has then attempts to update the
   * illness object's chronic attribute.
   *
   * @return Returns 'true' only if the illness chronic attribute was successfully changed, 'false'
   * otherwise.
   */
  ...
  /**
   * Checks if the nameOfDiagnoses GUI text field has changed, and if it has attempts to update the
   * illness object's name attribute.
   *
   * @return Returns 'true' only if the illness object's name attribute was successfully changed,
   * 'false' otherwise.
   */
  public boolean checkName() {
    boolean changes = false;
    if (!nameOfDiagnoses.getText().toUpperCase().equals(illness.getName())) {
      donor.updateIllness(AccountManager.getCurrentUser(), illness.getName(), "name",
          nameOfDiagnoses.getText());
      if (!updateMessageIsAnErrorMessage()) {
        changes = true;
      }
    }
  ...
  /**
   * Checks each Illness GUI field to see if any of the fields have been edited, and if they have,
   * update the corresponding illness attribute. If any inputted value is invalid, an error alert
   * box will inform the user of the violation. If No updates were made then the user is taken back
   * to the viewProfile page. If there were successful updates made, then the user will be informed
   * this was the case, and taking back to the viewprofile page.
   */
  public void editIllness() {
    ...
    if (nameChanged || dateChanged || chronicChanged || curedChanged) {
      showSuccessMessage(
          "Diagnosis successfully updated. Make sure to save the application to make the changes permanent.");
  ...
  /**
   * Iterates through the donor masterIllnessList and attempts to match the given name to an
   * Illness.
   *
  ...
  /**
   * Attempts to create an Illness object from the given Illness GUI fields. If any inputted value
   * is invalid, an error alert box will inform the user of the violation. If the new diagnosis was
   * successfully created then the user will be informed this was the case, and taking back to the
   * viewprofile page.
   */
  public void createIllness() {
    if (!illness.validateName(nameOfDiagnoses.getText())) {
      showBadInputMessage("ERROR: Invalid name" + nameOfDiagnoses.getText()
          + ". Name must be between 1 and 50 alphabetical characters.");
      return;
    } else if (checkNameUsed(nameOfDiagnoses.getText())) {
      showBadInputMessage(
          "ERROR: Duplicate name" + nameOfDiagnoses.getText() + ". This diagnosis already exists.");
      return;
    }
    ...
    if (!illness
        .compareChronicWithCured(chronicCheckBox.isSelected(), curedCheckBox.isSelected())) {
      showBadInputMessage("ERROR: A diagnosis cannot be both chronic and cured at the same time.");
      return;
    }

    Illness ill = new Illness(nameOfDiagnoses.getText().toUpperCase(), date,
        curedCheckBox.isSelected(), chronicCheckBox.isSelected());

    donor.getMasterIllnessList().add(ill);
    LogEntry logEntry = new LogEntry(donor, AccountManager.getCurrentUser(),
        "Illness '" + ill.getName() + "'", "non-existent", "created");
    donor.logChange(logEntry);
    showSuccessMessage(
        "New Diagnosis successfully created. Don't forget to save the application to make the change permanent.");
    illness = null;
    donor.populateIllnessLists();
    EditPaneController.setAccount(donor);
    goBackToEditPane();
