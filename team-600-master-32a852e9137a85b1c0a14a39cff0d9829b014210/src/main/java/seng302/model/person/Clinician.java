   ...
   /**
    ...
    public String updateGivenName(String value) {
        ...
        if (UserAttributeCollection.validateAlphanumericString(false, value, 1, 50)) {
            ...
            if(getFirstName() != null) { message = makeLog("givenName", getFirstName(), value); }
            else { message = makeLog("givenName", "unspecified", value); }
            setFirstName(value);
        }
        return message;
    }
    ...
    public String updateLastName(String value) {
        ...
        if (UserAttributeCollection.validateAlphanumericString(false, value, 1, 50)) {
            ...
            if(getLastName() != null) { message = makeLog("lastName", getLastName(), value); }
            else { message = makeLog("lastName", "unspecified", value); }
            setLastName(value);
        }
        return message;
    }
    ...
    public String updateWorkAddress(String value) {
        ...
        if (UserAttributeCollection.validateAlphanumericString(true, value, 1, 100)) {
            ...
            if(getContactDetails().getAddress().getStreetAddressLn1() != null) { message = makeLog("workAddress", getContactDetails().getAddress().getStreetAddressLn1(), value); }
            else { message = makeLog("workAddress", "unspecified", value); }
    ...
    public String updateRegion(String value) {
        String message = "";
        if(getContactDetails().getAddress().getRegion() != null) { message = makeLog("region", getContactDetails().getAddress().getRegion(), value); }
        else { message = makeLog("region", "unspecified", value); }
    ...
    /**
     * Creates a log String which documents an clinician attribute change. It records the old value, new value,
     * and the time of the change, as a string.
     * @param attribute A string of an attribute that will be changed. Will be one of "GivenName", "lastName", "region", or "workAddress".
     * @param oldValue A string of the attributes original value.
     * @param newValue A String of the attribute's new value.
     * @return A log string.
     */
    public String makeLog(String attribute, String oldValue, String newValue) {
        return DonorReceiver.createLog(attribute, oldValue, newValue) + "\n";
    }
