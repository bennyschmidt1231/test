...
public class MedicalProcedure {
    ...
    private static ArrayList<String> organNames = new ArrayList<>(Arrays.asList("Liver", "Kidney", "Pancreas", "Heart",
            "Lung", "Intestines", "Cornea", "Middle Ear", "Skin", "Bone", "Bone Marrow", "Connective Tissue"));
     ...
     * @param affectedOrgans An ArrayList containing the names of the organs that are affected by the procedure.
     */
    @JsonCreator
     ...
     * @param dob The person's date of birth. This is the person that the procedure will be/was performed on.
     ...
     * @throws Exception Throws exception  with message {@link #invalidDateErrorMessage} if the date provided is not a
     *                   valid date.
     *                   Throws exception with message {@link #nullDOBErrorMessage} if the date of birth provided is null.
     *                   Throws exception with message {@link #procedureDateTooEarlyErrorMessage} if the procedure is
     *                   more than 12 months before the date of birth.
     *                   Throws exception with message {@link #invalidOrgansErrorMessage} if the organ names in
     *                   affectedOrgans are not valid (i.e. not in the list of valid organ names {@link #getOrganNames()}).
     */
    public MedicalProcedure(String summary, String description, String dateString, LocalDate dob,
        ...
        if (!validAffectedOrgans(affectedOrgans)) throw new Exception(invalidOrgansErrorMessage);
        else this.affectedOrgans = new ArrayList<>(affectedOrgans);
    }
    ...
    /**
     * Sets the date of the procedure. The new date must be no more than 12 months before the person's date of birth.
     * @param dateString The string value of the date. Must be in format "dd/mm/yyyy".
     * @param dob The person's date of birth. This is the person that the procedure is/was performed on.
     *            Can be null if there is no date confirmed for the procedure.
     * @throws Exception Throws exception  with message {@link #invalidDateErrorMessage} if the date provided is not a
     *                   valid date.
     *                   Throws exception with message {@link #nullDOBErrorMessage} if the date of birth provided is null.
     *                   Throws exception with message {@link #procedureDateTooEarlyErrorMessage} if the procedure is
    ...
    public void setDate(String dateString, LocalDate dob) throws Exception{
        if (dateString == null) {
            ...
            if (!date.isBefore(dob.minusYears(1))) {
                this.date = date;
            } else throw new Exception(procedureDateTooEarlyErrorMessage);
        }
    }

    /**
     * The getter method for the list of affected organs of this procedure. The valid organ names are: "Liver",
     * "Kidney", "Pancreas", "Heart", "Lung", "Intestines", "Cornea", "Middle Ear", "Skin", "Bone", "Bone Marrow",
     * and "Connective Tissue". See {@link #getOrganNames()}.
     * @return The affected organs of this procedure.
     */
    public ArrayList<String> getAffectedOrgans() { return new ArrayList<>(affectedOrgans); }

    /**
     * The setter method for the list of affected organs of this procedure. The valid organ names are: "Liver",
     * "Kidney", "Pancreas", "Heart", "Lung", "Intestines", "Cornea", "Middle Ear", "Skin", "Bone", "Bone Marrow",
     * and "Connective Tissue". See {@link #getOrganNames()}.
     * @param affectedOrgans The list of affected organs of this procedure.
     * @throws Exception Throws exception with message {@link #invalidOrgansErrorMessage} if the organ names in
     *                   affectedOrgans are not valid (i.e. not in the list of valid organ names {@link #getOrganNames()}).
     */
    public void setAffectedOrgans(ArrayList<String> affectedOrgans) throws Exception{
        if (!MedicalProcedure.validAffectedOrgans(affectedOrgans)) throw new Exception(invalidOrgansErrorMessage);
        else this.affectedOrgans = new ArrayList<>(affectedOrgans);
    }
    ...
    /**
     * Tests whether all the organ names given by affectedOrgans are in the list of valid organ names. See
     * {@link #getOrganNames()} for a list of valid organ names.
