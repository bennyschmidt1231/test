...
/**
 * The controller class for all viewing or editing functionality for the current logged in clinician in the App.w
 */
public class ClinicianProfileController {
    ...
    @FXML private TextField clinicianProfileNameText;
    @FXML private TextField lastNameText;
    @FXML private TextArea workAddressText;
    @FXML private Button doneButton;
    @FXML private Label staffIDLabel;
    @FXML private Label dateLabel;
    @FXML private ComboBox<String> regionComboBox;
    @FXML private ListView view;
    ...
    private final ArrayList<String> ignoredKeys = new ArrayList<>(Arrays.asList("ESCAPE", "F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9", "F10", "F11", "F12", "PRINTSCREEN", "SCROLL_LOCK", "PAUSE", "PAGE_UP", "HOME", "INSERT", "END", "PAGE_DOWN", "NUM_LOCK", "ENTER", "RIGHT", "UP", "DOWN", "LEFT", "CONTROL", "CONTEXT_MENU", "ALT_GRAPH", "ALT", "WINDOWS", "CONTROL", "SHIFT", "CAPS"));
    ...
    public static void setClinician(Clinician newClinician) {clinician = newClinician;}

    public static Clinician getClinician() {return clinician;}
    ...
    /**
     * Initializes all the attributes seen by the user when they load the View/Edit Clinician GUI page. This includes the
     * current clinician's details as well as a log of any modifications they have made to their profile.
     */
    ...
    private void loadEditFields() {
        ...
        regionComboBox.getSelectionModel().select(clinician.getContactDetails().getAddress().getRegion());
    ...
    /**
     * Checks each text field in the View/Edit Clinician GUI for any changes to the attributes and attempts to validate,
     * then update, then finally applies any changes made. Afterwards the viewing element are shown. If any
     * of the updates were invalid, the user will be informed about the violation in an alert pop-up box.
     */
    @FXML void doneButtonPressed() {
        boolean changes = false;
        if (!clinicianProfileNameText.getText().equals(clinician.getFirstName())) {
            LogEntry logEntry = new LogEntry(clinician, AccountManager.getCurrentUser(), "firstName", clinician.getFirstName(), clinicianProfileNameText.getText());
        ...
        if (!lastNameText.getText().equals(clinician.getLastName())) {
            LogEntry logEntry = new LogEntry(clinician, AccountManager.getCurrentUser(), "lastName", clinician.getLastName(), lastNameText.getText());
        ...
        if (!workAddressText.getText().equals(clinician.getContactDetails().getAddress().getStreetAddressLn1())) {
            LogEntry logEntry = new LogEntry(clinician, AccountManager.getCurrentUser(), "workAddress", clinician.getContactDetails().getAddress().getStreetAddressLn1(), workAddressText.getText());
        ...
        if (region != null && !region.equals(clinician.getContactDetails().getAddress().getRegion())) {
            LogEntry logEntry = new LogEntry(clinician, AccountManager.getCurrentUser(), "region", clinician.getContactDetails().getAddress().getRegion(), region);
    ...
    public static void undoCalled() {
        ...
        if (!current.getUndo().empty() && current.getUndo().peek().getUndoRedoName().equals("Label")) undoRedoLabel = true;
        if (!current.getUndo().empty() && current.getUndo().peek().getUndoRedoName().equals("Text Area")) undoRedoTextArea = true;
    ...
    public static void redoCalled() {
        ...
        if (!current.getRedo().empty() && current.getRedo().peek().getUndoRedoName().equals("Label")) undoRedoLabel = true;
        if (!current.getUndo().empty() && current.getUndo().peek().getUndoRedoName().equals("Text Area")) undoRedoTextArea = true;
