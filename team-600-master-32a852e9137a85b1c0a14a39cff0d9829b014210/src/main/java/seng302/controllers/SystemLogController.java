...
/**
 * SystemLogController is a JavaFX controller used in conjunction with the FXML
 * file SystemLog to produce an interactive pane.
 */
public class SystemLogController {


    // Class attributes.
    @FXML private ListView<String> logDisplay;
    private final char DATE_ENTRY_CHARACTER = 0;
    private final Background BACKGROUND_STYLE = new Background(new BackgroundFill(Paint.valueOf("lightgrey"), CornerRadii.EMPTY, Insets.EMPTY));


    /**
     * Adds a listener to the system log which will call the refreshList method
     * each time the system log is changed.
     */
    ...
    /**
     * Returns a new ObservableList which contains the log entries in the
     * original list sorted in descending order with day entries added. A day
     * entry consists of a null char used in cell processing and the date.
     *
     * @param logEntries The system log in which day entries are to be inserted.
     * @return A new ObservableList containing the original log entries sorted
     * with day entries inserted every time the date changes.
     */
    private ObservableList<String> addDayEntries(List<LogEntry> logEntries) {
        ...
        if (logEntries.get(0).getChangeTime().toLocalDate().equals(date)) {
    ...
    /**
     * Initialises the system log pane. Populates the list with log entries and
     * adds a listener to the system log to monitor changes.
     */
    ...
    /**
     * Initializes the row factory used to display log entries. If an entry is
     * a day entry, specified by a null char in the first position, it will be
     * highlighted light grey. Otherwise, the cell will display normally.
     */
